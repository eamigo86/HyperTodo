...
| --- | --- |
| Focused, ready `mode="list"`, exactly page 1 | Automatic whole-document reload. |
| More than one loaded page, or a page other than 1 | Persistent notice; deliberate Update returns to page 1. |
| `mode="notice"`, including forms | Preserve draft until deliberate Update. |
| Hidden route | Defer; on focus return apply that same route's policy. |
...
filters and sets page 1 for list policy. Notice/form targets preserve their
...
`csrf` and `error`. Each must be nonblank plain text of at most 512 UTF-16 code units.
...
uses that neutral copy through the same resource policy. Separate captured change
...
The notice is persistent and accessible, with an alert/live region and an explicit
Update button, outside FlatList's item collection. Pressing Update while work is
...
these JavaScript tests; notice styling is a small explicitly styled readable panel.
