...
| --- | --- | --- | --- |
| P1 | Explicit document and fragment responses | Several mutations produced `XMLRestrictedElementFound` after returning `doc` to a replacement target. | Add an explicit fragment response/template response with the fragment media type and response-shape validation. Keep the existing document response backward compatible. |
